...
*請在回應中保留 Markdown 格式，特別是鏈接和用戶提及*`;
  ...
  function displayModelsList(models) {
    ...
    const modelsHTML = models.map(model => {
      ...
      const badgeHTML = isDefaultModel ? 
        '<span class="model-badge default">預設</span>' : 
        '<span class="model-badge synced">同步</span>';
  ...
  function triggerManualSync() {
    ...
    chrome.runtime.sendMessage({ action: 'triggerBackgroundSync' }, (response) => {
      if (chrome.runtime.lastError) {
        showSyncingError('手動同步失敗: ' + chrome.runtime.lastError.message);
        ...
        showSyncingError('手動同步失敗: ' + (response?.error || '未知錯誤'));
  ...
  function startSyncTimer() {
    ...
    syncTimer = setInterval(() => {
      ...
      timerElement.textContent = `已用時間: ${elapsed} 秒`;
    }, 1000);
  }
  ...
  function simulateSyncProgress() {
    const steps = [
      { id: 'step1', delay: 500, message: '正在開啟 Gemini 頁面...' },
      { id: 'step2', delay: 2000, message: '頁面載入中，請稍候...' },
      { id: 'step3', delay: 4000, message: '正在檢測並提取可用模型...' },
      { id: 'step4', delay: 6000, message: '正在保存模型列表到本地存儲...' }
    ];
    ...
    // 在最後一步後等待實際同步完成
    setTimeout(() => {
      updateSyncMessage('同步即將完成，正在等待後端處理...');
    }, 7000);
  }
  ...
  function showSyncingSuccess() {
    ...
    if (title) {
      title.textContent = '同步成功完成！';
    }
    
    updateSyncMessage('✅ 模型同步成功！正在更新本地模型列表...');
    ...
    if (syncTimer) {
      ...
      syncTimer = null;
      const elapsed = Math.floor((Date.now() - syncStartTime) / 1000);
      document.getElementById('syncTimer').textContent = `總用時: ${elapsed} 秒`;
    }
  }
